        ...
        for context_length in tqdm(context_lengths, desc="Context Lengths"):
            for target_length in tqdm(target_lengths, desc=f"Processing Context Length: {context_length}", leave=False):
                ...
                for i, prompts in enumerate(tqdm(self.dataset, desc=f"Processing Context Length: {context_length}, Target Length: {target_length}", leave=False)): 
                    ...
                    self.samples_processed += len(output_ids)
                    ...
                    # print(f"inputs device: {inputs.input_ids.device}")
                        # Save the current batch's results immediately
                    df = pd.DataFrame(batch_results)
                    df.to_csv(f"{self.log_path}/results_batch_{self.current_batch}_context{context_length}_target{target_length}.csv", index=False)
                    self.current_batch += 1
                    
                    import subprocess
                    subprocess.run("git remote -v", shell=True)
                    subprocess.run("git add logs", shell=True)
                    subprocess.run('git commit -m "Auto update results added"', shell=True)
                    ...
                    if self.samples_processed >= num_samples:
                        final_ratio = self.memorized / self.samples_processed
                        print(f"Memorized: {final_ratio}")
                    
                        # Save the final summary separately without appending it to batch_results
                        results_summary = {
                            f"Model={self.model_name}_Context={context_length}_Target={target_length}": {
                                "Context Length": context_length,
                                "Target Length": target_length,
                                "Memorized Ratio": final_ratio,
                                "Index List": self.memorized_list,
                                "Outlier List": self.outlier_list,
                                "Total Samples": self.samples_processed
                            }
                        }
                        ...
                        subprocess.run("git remote -v", shell=True)
                        subprocess.run("git add logs", shell=True)
                        subprocess.run('git commit -m "Auto update final summary added"', shell=True)
                        subprocess.run("git push -u origin main", shell=True)
