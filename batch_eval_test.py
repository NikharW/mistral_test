...
from typing import Optional, Iterator, Dict, List, Callable
import subprocess
...
class MemorizationAnalyser:
    def __init__(
        ...
        output_after_batch: bool = False,
        on_batch_complete: Optional[str] = None,
    ):
        ...
        self.output_after_batch = output_after_batch
        ...
        if (quant_config_swap is None and (layer_swap_config is not None or swap_every is not None)) or \
            ...
            raise ValueError(f"Please provide both quant_config_swap and either layer_swap_config or swap_every,\
                            but not both.")
    def save_batch_results(self, results: List, context_length: int, target_length: int) -> None:
        """Save results for current batch and trigger completion hook if specified"""
        try:
            # Save results to CSV
            df = pd.DataFrame(results)
            batch_file = f"{self.log_path}/batch_{self.current_batch}_results.csv"
            df.to_csv(batch_file, index=False)
            
            # Save summary
            current_summary = {
                f"Model={self.model_name}_Context={context_length}_Target={target_length}": {
                    "Context Length": context_length,
                    "Target Length": target_length,
                    "Current Memorized Ratio": self.memorized / max(1, self.prompts),
                    "Batch": self.current_batch,
                    "Samples Processed": self.samples_processed
                }
            }
            with open(f"{self.log_path}/batch_{self.current_batch}_summary.json", "w") as f:
                json.dump(current_summary, f, indent=4)
                
            # Execute batch completion command if specified
            if self.on_batch_complete:
                try:
                    subprocess.run(
                        f"{self.on_batch_complete} {self.current_batch}", 
                        shell=True, 
                        check=True,
                        capture_output=True,
                        text=True
                    ...
                    print(f"Error executing batch completion command: {e}")
        ...
        for context_length in tqdm(context_lengths, desc="Context Lengths"):
            for target_length in tqdm(target_lengths, desc=f"Processing Context Length: {context_length}", leave=False):
                ...
                for i, prompts in enumerate(tqdm(self.dataset, desc=f"Processing Context Length: {context_length}, Target Length: {target_length}", leave=False)): 
                    ...
                    self.samples_processed += len(prompts['text'])
                    if self.samples_processed >= (self.current_batch + 1) * self.batch_size:
                        self.current_batch += 1
                        if self.output_after_batch:
                            self.save_batch_results(results, context_length, target_length)
        
                    if self.samples_processed >= num_samples:
                        break
...
def run_analysis(
    ...
    analyzer = MemorizationAnalyser(
        ...
        output_after_batch=output_after_batch,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-after-batch",
        action="store_true",
        help="Save results after each batch is completed"
    )
    parser.add_argument(
        "--on-batch-complete",
        type=str,
        help="Shell command to execute after each batch completes. Batch number will be appended."
    )
