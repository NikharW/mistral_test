...
python batch_push.py --model-config configs/models_$MODEL_NUM.json --quant-config configs/baseline-config.json --dataset "monology/pile-uncopyrighted" --num-samples 100 --batch-size 4
...
for swap in "${swap_values[@]}"; do
    ...
    python batch_push.py --model-config configs/models_$MODEL_NUM.json \
