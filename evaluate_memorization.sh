...
python batch_push.py --model-config configs/model_$MODEL_NUM.json --quant-config configs/baseline-config.json --dataset "monology/pile-uncopyrighted" --num-samples 10000 --batch-size $BATCH_SIZE
...
for swap in "${swap_values[@]}"; do
    ...
    python batch_push.py --model-config configs/model_$MODEL_NUM.json \
